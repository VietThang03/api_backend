...
import Hastag from "~/models/schemas/Hashtag.schema"
...
const MYID = new ObjectId('64ba801a5242d95d7f725627')
...
const createRandomStatus = () => {
  const status: StatusRequestBody = {
    type: StatusTypeEnum.Status,
    audience: StatusAudience.Everyone,
...
const checkAndCreateHashtags = async (hashtags: string[]) => {
  const hashtagDocuemts = await Promise.all(
    hashtags.map((hashtag) => {
      ...
      return database.hastags.findOneAndUpdate(
        { name: hashtag },
        {
          $setOnInsert: new Hastag({ name: hashtag })
        },
        {
          upsert: true,
  ...
  return hashtagDocuemts.map((hashtag) => (hashtag.value as WithId<Hastag>)._id)
}

const insertTweet = async (user_id: ObjectId, body: StatusRequestBody) => {
...
const insertMultipleTweets = async (ids: ObjectId[]) => {
  ...
  const result = await Promise.all(
    ids.map(async (id, index) => {
      await Promise.all([insertTweet(id, createRandomStatus()), insertTweet(id, createRandomStatus())])
      count += 2
