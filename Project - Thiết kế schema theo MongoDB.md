# Thiết kế Schema dự án bằng MongoDB

## Phân tích chức năng các collection
...
- Sau khi đăng ký xong thì sẽ có email đính kèm `email_verify_token` để xác thực email (`hoinhagiau.com/verify-email?email_verify_token=123321123`). Mỗi user chỉ có 1 `email_verify_token` duy nhất, vì nếu user nhấn re-send email thì sẽ tạo ra `email_verify_token` mới thay thế cái cũ. Vậy nên ta lưu thêm trường `email_verify_token` vào schema `users`. Trường này có kiểu `string`, nếu user xác thực email thì ta sẽ set `''`.

- Tương tự ta có chức năng quên mật khẩu thì sẽ gửi mail về để reset mật khẩu, ta cũng dùng `forgot_password_token` để xác thực (`hoinhagiau.com/forgot-password?forgot_password_token=123321123`). Vậy ta cũng lưu thêm trường `forgot_password_token` vào schema `users`. Trường này có kiểu `string`, nếu user reset mật khẩu thì ta sẽ set `''`.

- Nên có một trường là `verify` để biết trạng thái tài khoản của user. Ví dụ chưa xác thực email, đã xác thực, bị khóa, lên tích xanh ✅. Vậy giá trị của nó nên là enum (có thể nâng cấp lên chức năng này)
...
1. bài đăng có thể chứa text, hashtags, metions, ảnh, video
2. bài đăng có thể hiển thị cho everyone hoặc private
3. bài đăng có thể quy định người reply (everyone, người mà chúng ta follow , người chúng ta metion)

- Nếu là bài đăng bình thường thì sẽ có `content` là string. Còn nếu không có `content` lúc này có thể cho content là `''` hoặc `null`. Vậy nên `content` có thể là `string`.

> Nếu là '' thì sẽ chiếm bộ nhớ hơn là null, nhưng điều này là không đáng kể so với lợi ích nó đem lại

- `audience` đại diện cho tính riêng tư của bài đăng. Ví dụ bài đăng có thể là public cho mọi người xem hoặc chỉ cho nhóm người nhất định. Vậy nên `visibility` có thể là `StatusAudience` enum.

- `hashtag` là mảng chứa ObjectId của các hashtag. Vì mỗi bài đăng có thể có nhiều hashtag. Vậy nên `hashtag` có thể là `ObjectId[]`.

- `mentions` là mảng chứa ObjectId của các user được mention. Vì mỗi bài đăng có thể có nhiều user được mention. Vậy nên `mentions` có thể là `ObjectId[]`.

- `medias` là mảng chứa ObjectId của các media. Vì mỗi bài đăng chỉ có thể có 1 vài media. Nếu upload ảnh thì sẽ không upload được video và ngược lại. Vậy nên `medias` có thể là `Media[]`.
==> Video upload chỉ trong quá trình nghiên cứu :>

  Lượt view thì chúng ta chia ra làm 2 loại là `guest_views` là số lượng lượt xem của bài đăng từ người dùng không đăng nhập và `user_views` là dành cho đã đăng nhập. 2 trường này sẽ cho kiểu dữ liệu là `number`.
...
Bookmark các bài đăng lại, mỗi user không giới hạn số lượng bookmark. Sở dĩ không cần `updated_at` là vì trong trường hợp người dùng unbookmark thì chúng ta sẽ xóa document này đi.
...
interface Status {
  ...
  audience: StatusAudience
  content: string
  ❌hashtags:string[] // Không nên nhúng như thế này, vì sẽ gây khó khăn trong việc tìm kiếm những bài đăng nào có hashtag này, cũng như là gây lặp lại dữ liệu về tên hastag
  mentions: ObjectId[]
  medias: Media[]
  guest_views: number
  user_views: number
...
- Lưu một array ObjectId `hashtags` trong collection `statuss`

- Tạo ra một collection riêng để lưu `hashtags` và không lưu mảng `status_id` vào trong collection `hashtags`. Vì nếu lưu `status_id` vào trong collection `hashtags` thì sẽ dễ chạm đến giới hạn 16MB của MongoDB. Và cũng không cần thiết để lưu, vì khi search các bài đăng liên quan đến hashtag thì chúng ta sẽ dùng id hashtag để tìm kiếm trong collection `statuss`.
...
- giả sử một trường hợp tạo bài đăng đầy đủ hashtag, mention và media
  ...
  "$jsonSchema": {
    ...
    "properties": {
      ...
      "audience": {
        "bsonType": "int",
        "enum": [0, 1],
        "description": "'audience' must be a StatusAudience and is required"
      },
      "content": {
        "bsonType": "string",
